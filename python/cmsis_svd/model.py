...
from typing import List, Optional, Any, Union, TypedDict, Generator
...
class SVDJSONEncoder(json.JSONEncoder):
    _TO_DICT_SKIP_KEYS = {'parent'}

    def default(self, o: Any) -> Any:
        if isinstance(o, SVDElement):
            eldict = {}
            for k, v in six.iteritems(o.__dict__):
                if k in self._TO_DICT_SKIP_KEYS:
                    continue
                if k.startswith("_"):
                    pubkey = k[1:]
                    eldict[pubkey] = getattr(o, pubkey)
                else:
                    eldict[k] = v
            return eldict
...
class SVDRegisterPropertiesGroup:
    ...
    protection: Optional[str] = field(default=None)
...
class SVDSauRegionsConfigRegion(SVDElement):
    ...
    access: Optional[str] = field(default=None, metadata={'required': True})
...
class SVDSauRegionsConfig(SVDElement):
    ...
    protection_when_disabled: Optional[str] = field(default=None, metadata={'type': 'attribute'})
