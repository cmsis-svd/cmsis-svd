...
from cmsis_svd.model import (
    ...
    SVDAccessType, SVDAddressBlockUsageType, SVDDataTypeType,
    SVDModifiedWriteValuesType, SVDReadActionType, SVDFieldArray,
    SVDDimArrayIndex, SVDEndianType, SVDSauRegionsConfig,
...
class SVDParser:
    ...
    @classmethod
    def for_xml_file(cls, path: str) -> 'SVDParser':
    ...
    def for_packaged_svd(
        cls, package_root: str, vendor: str, filename: str
    ...
    @classmethod
    def for_mcu(cls, package_root: str, mcu: str) -> Union['SVDParser', None]:
    ...
    @staticmethod
    def _parse_dim_index(text_value: str) -> Union[List[str], None]:
    ...
    def _parse_dim_element_group(self, node: Element) -> DimElementGroupType:
        ...
        dim_index = self._parse_dim_index(_get_text(node, 'dimIndex'))
    ...
    def _parse_address_block(
        ...
        usage = None
        if (usage_text := _get_text(address_block_node, 'usage')) is not None:
        ...
        return SVDAddressBlock(
            ...
            usage=usage,
            protection=_get_text(address_block_node, 'protection')
        )
    ...
    def _parse_enumerated_values(
        ...
        usage = None
        if (usage_text := _get_text(enumerated_values_node, 'usage')) is not None:
            usage = self._parse_usage_type(usage_text)
    ...
    def _parse_register(
        ...
        fields = [self._parse_field(f) for f in register_node.findall('./fields/field')
                  if not self.remove_reserved or 'reserved' not in f.name.lower()]
    ...
    def _parse_sau_regions_config_region(
        ...
        return SVDSauRegionsConfigRegion(
            ...
            access=_get_text(region_node, 'access')
        )
    ...
    def _parse_sau_regions_config(
        ...
        return SVDSauRegionsConfig(
            ...
            protection_when_disabled=sau_region_config_node.get('protectionWhenDisabled'),
            regions=regions
        )
